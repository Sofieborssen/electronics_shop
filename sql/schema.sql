...
-- För psql: Kör detta skript för att skapa databasen och tabellerna
-- psql -U <användarnamn> -f schema.sql

-- Skapa databasen (kräver att användaren har behörighet)
-- Om databasen redan finns, hoppa över detta steg
SELECT 'CREATE DATABASE electronics_db'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'electronics_db')\gexec
